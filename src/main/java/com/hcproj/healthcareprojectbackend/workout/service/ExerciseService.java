...
public class ExerciseService {
     ...
     */
    @Transactional(readOnly = true)
    public ExerciseListResponseDTO getExerciseList(Long cursor, Integer limit, String keyword, List<String> bodyParts, List<String> difficulties) {
        int actualLimit = (limit == null || limit <= 0) ? DEFAULT_LIMIT : Math.min(limit, MAX_LIMIT);
        int limitSize = actualLimit + 1;

        String normalizedKeyword = UtilityProvider.normalizeKeyword(keyword);
        List<String> normalizedBodyParts = UtilityProvider.normalizeTokens(bodyParts);
        List<String> normalizedDifficulties = UtilityProvider.normalizeTokens(difficulties);

        if (normalizedBodyParts != null && normalizedBodyParts.isEmpty()) normalizedBodyParts = null;
        if (normalizedDifficulties != null && normalizedDifficulties.isEmpty()) normalizedDifficulties = null;

        Pageable pageable = PageRequest.of(0, limitSize);

        List<ExerciseEntity> entities;
        if (normalizedKeyword == null) {
            entities = exerciseRepository.findPageNoKeyword(
                    cursor, normalizedBodyParts, normalizedDifficulties, pageable
            );
        } else {
            String likePattern = "%" + normalizedKeyword.toLowerCase().replace(" ", "") + "%";
            entities = exerciseRepository.findPageWithKeyword(
                    cursor, likePattern, normalizedBodyParts, normalizedDifficulties, pageable
            );
        }

        boolean hasNext = entities.size() > actualLimit;
        List<ExerciseEntity> result = hasNext ? entities.subList(0, actualLimit) : entities;

        List<ExerciseListResponseDTO.ExerciseItemDTO> items = result.stream()
                .map(e -> new ExerciseListResponseDTO.ExerciseItemDTO(
                        e.getExerciseId(),
                        e.getName(),
                        e.getImageUrl(),
                        e.getBodyPart(),
        ...
        Long nextCursor = hasNext && !result.isEmpty()
                ? result.get(result.size() - 1).getExerciseId()
                : null;
